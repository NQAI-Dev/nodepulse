...
// legacyMasterTokenLiteral is the well-known admin token that older
// nodepulse binaries (pre-master-token-rotation) embedded directly in
// source. It is referenced here ONLY for one-time migration detection:
// if we find this exact string in api_tokens on startup, we rotate it
// to a freshly generated random value. After that first rotation, the
// constant is never again relevant to a running binary — the running
// process only ever compares incoming tokens against the random value
// stored in its in-memory cache.
const legacyMasterTokenLiteral = "np_live_master_secret"
...
type PersistentStore struct {
	...
	// masterToken is the cached value of the api_tokens row where
	// name='master'. Populated at startup by ensureMasterToken. Used by
	// ValidateToken / GetUserByToken / IsMasterToken so the "break
	// glass" admin fallback works even when the DB itself is wedged.
	// The literal credential never lives in source; see
	// legacyMasterTokenLiteral for the migration-detection constant.
	masterToken string
...
//
//  1. No row exists: generate a fresh 256-bit random token, INSERT it,
//     log the value ONCE so the operator can capture it.
//  2. Row exists and matches the legacy literal: rotate to a fresh
//     random value (UPDATE), log the new value ONCE. This closes the
//     attack surface where anyone with read access to a pre-fix
//     binary could grep for the literal and use it as a credential.
//  3. Row exists with a non-literal value: return as-is. The token was
//     generated by a previous startup (or rotated by operator).
//
// The token value is NOT logged beyond the first startup-after-rotation;
// subsequent restarts read the existing row silently. Anyone with DB
// read access can extract it, but the DB is already trusted for all
// other token auth — this is no different.
func ensureMasterToken(db *sql.DB, adminID int64) (string, error) {
	var existing string
	err := db.QueryRow(
		"SELECT token FROM api_tokens WHERE user_id = ? AND name = 'master'",
		adminID,
	).Scan(&existing)
	switch {
	case err == sql.ErrNoRows:
		// Branch 1: fresh install — no master row yet.
		fresh, err := generateMasterToken()
		if err != nil {
			return "", fmt.Errorf("generate fresh master token: %w", err)
		}
		if _, err := db.Exec(
			"INSERT INTO api_tokens (token, user_id, name) VALUES (?, ?, 'master')",
			fresh, adminID,
		); err != nil {
			return "", fmt.Errorf("insert master token: %w", err)
		}
		log.Printf("[nodepulse] generated master admin token — capture now, shown once: %s", fresh)
		return fresh, nil
	case err != nil:
		return "", fmt.Errorf("query master token: %w", err)
	default:
		if existing == legacyMasterTokenLiteral {
			// Branch 2: rotate legacy literal away on first startup after
			// the master-token-rotation fix lands.
			fresh, err := generateMasterToken()
			if err != nil {
				return "", fmt.Errorf("generate rotated master token: %w", err)
			}
			if _, err := db.Exec(
				"UPDATE api_tokens SET token = ? WHERE user_id = ? AND name = 'master'",
				fresh, adminID,
			); err != nil {
				return "", fmt.Errorf("update legacy master token: %w", err)
			}
			log.Printf("[nodepulse] rotated legacy master admin token — capture now, shown once: %s", fresh)
			return fresh, nil
		}
		// Branch 3: existing non-literal token, keep as-is.
		return existing, nil
	}
}

// generateMasterToken produces a 256-bit cryptographically random token,
// hex-encoded, prefixed for audit-log readability.
func generateMasterToken() (string, error) {
	buf := make([]byte, 32)
	if _, err := rand.Read(buf); err != nil {
		return "", err
	}
	return "np_master_" + hex.EncodeToString(buf), nil
}
...
func NewPersistentStore(dbPath string, botToken string, chatID int64) (*PersistentStore, error) {
	...
	if err != nil {
		...
		if err == nil {
			...
			// Master-token row is created by ensureMasterToken below,
			// outside the admin-creation branch so it runs on every
			// startup (including the legacy case where admin already
	...
	// value is consumed by the &PersistentStore{...} literal further
	// down (after the rest of the schema is created) so the "break
	// glass" admin auth path works even when subsequent DB lookups
	// fail. Three branches:
	//   - no row: generate a fresh random token, INSERT, log ONCE.
	//   - row matches the legacy literal: rotate to fresh random, log
	//     ONCE. This closes the pre-fix attack surface where anyone
	//     with read access to the binary could grep for the literal.
	//   - row holds a non-literal value: keep as-is, return.
	// The literal itself never lives in source as a credential — it is
	// only a migration marker, compared exactly once at startup.
	masterTok, err := ensureMasterToken(db, adminID)
	if err != nil {
		return nil, fmt.Errorf("ensure master token: %w", err)
	}
...
func (p *PersistentStore) GetUserByToken(token string) (int64, string, error) {
	if subtle.ConstantTimeCompare([]byte(token), []byte(p.masterToken)) == 1 {
		return 1, "admin", nil
	}
	var uid int64
	var uname string
...
// IsMasterToken reports whether the supplied token matches the cached
// master admin token. Used by /api/v1/ingest and /api/v1/autoheal/log to
// allow break-glass admin requests through even when GetUserByToken (which
